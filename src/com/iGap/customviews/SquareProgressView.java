...
/**
 * 
 * for set progress in view
 * 
 * @Use in all of list items in all of chats
 *
 */
...
public class SquareProgressView extends View {
    ...
    private boolean      clearOnHundred      = true;
    private boolean      isIndeterminate     = false;
    private int          indeterminate_count = 1;
    private float        indeterminate_width = 100.0f;


    public SquareProgressView(Context context, View view) {
        super(context);
    ...
    private void initializePaints(Context context) {
        ...
        progressBarPaint.setColor(Color.parseColor("#2bbfbd"));//#70707070
        progressBarPaint.setStrokeWidth(CalculationUtil.convertDpToPx(widthInDp, getContext()));
        progressBarPaint.setAntiAlias(true);
        progressBarPaint.setDither(true); // set the dither to true
        progressBarPaint.setStrokeJoin(Paint.Join.ROUND); // set the join to round you want
        progressBarPaint.setStrokeCap(Paint.Cap.ROUND); // set the paint cap to round too
    ...
    protected void onDraw(Canvas canvas) {
        ...
        progressBarPaint.setPathEffect(new CornerPathEffect(canvas.getWidth() / 20));
        ...
        if (isIndeterminate()) {
            ...
            if (drawEnd.place == Place.TOP) {
                if (drawEnd.location > (canvas.getWidth() / 2)) {
                    ...
                    path.lineTo(drawEnd.location + 90, strokewidth / 2);
                }
                canvas.drawPath(path, progressBarPaint);
            }
