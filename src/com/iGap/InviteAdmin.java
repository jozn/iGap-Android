...
/**
 * 
 * ezafe kardan ozve admin be azaye kanal
 *
 */

public class InviteAdmin extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        connectionServiceBound = new ConnectionServiceBound(InviteAdmin.this);
        ...
        try {
            if (check.size() > 0) {
                customAdapterList = new customAdapterList(InviteAdmin.this, contactavatar_lqregistered, contactnameregistered, check, lastseenregistered);
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                connectionServiceBound.getLastSeen(contactmobileregistered, "InviteAdmin");
            ...
            public void onClick(View v) {
                ...
                if (check.size() > 0) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                        new SendNumbers().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                    } else {
        ...
        try {
            if (name != null) {
                if (name.trim().length() > 0) {
                    ...
                    customAdapterList = new customAdapterList(InviteAdmin.this, avatar_lq, names, checks, lastseenregistered);
                    registeredlv.setAdapter(customAdapterList);
    ...
    class SendNumbers extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            pDialog = new Dialog(InviteAdmin.this);
        ...
        protected String doInBackground(String... args) {
            try {
                ...
                JSONObject jsonobj = jParser.getJSONFromUrl(G.addmembertochannel + channeluid + "/invite-admin", params, "BODY", G.basicAuth, array.toString());
                ...
                try {
                    ...
                    if (success) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                Toast.makeText(InviteAdmin.this, getString(R.string.invite_admin_successful_en), Toast.LENGTH_SHORT).show();
                                finish();
                            }
                        });
                        ...
                        if (statuscode.equals("403")) {
                            ...
                            if (errorStatus.equals("1")) {
                                runOnUiThread(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        Toast.makeText(InviteAdmin.this, getString(R.string.channel_deleted), Toast.LENGTH_SHORT).show();
                                    }
                                });
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        Toast.makeText(InviteAdmin.this, getString(R.string.channel_closed), Toast.LENGTH_SHORT).show();
                                    }
                                });
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        Toast.makeText(InviteAdmin.this, getString(R.string.channel_invite_member_limitation), Toast.LENGTH_SHORT).show();
                                    }
                                });
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        Toast.makeText(InviteAdmin.this, getString(R.string.Error_occurred_en), Toast.LENGTH_SHORT).show();
                                    }
                                });
                            }
    ...
    protected void onResume() {
        Utils.checkLanguage(this);
        LocalBroadcastManager.getInstance(InviteAdmin.this).registerReceiver(UpdateLastInviteAdmin, new IntentFilter("UpdateLastInviteAdmin"));
        super.onResume();
        G.appIsShowing = true;
    }
    ...
    protected void onPause() {
        LocalBroadcastManager.getInstance(InviteAdmin.this).unregisterReceiver(UpdateLastInviteAdmin);
    ...
    private BroadcastReceiver UpdateLastInviteAdmin = new BroadcastReceiver() {
